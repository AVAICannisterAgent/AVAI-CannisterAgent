...
class CanisterAgent(ToolCallAgent):
    ...
    def __init__(self, **kwargs):
        ...
        self.devnet_uploader = DevNetUploader()
...
Focus on providing comprehensive, professional-grade IC canister analysis."""
    ...
    async def run(self, prompt: str = None) -> str:
        ...
        try:
            ...
            # Upload report to DevNet
            await self._upload_report_to_devnet(report, analysis_result, repo_url)
    ...
    async def _upload_report_to_devnet(
        self, 
        report_content: str, 
        ...
        """
        Upload the generated audit report to IC DevNet.
        ...
        """
        try:
            logger.info("🌐 [DEVNET] Starting DevNet upload process...")
            
            # Prepare metadata for upload
            metadata = {
                "repository_url": repo_url,
                "analysis_date": analysis_result.get("extraction_timestamp", "Unknown"),
                "analysis_duration": analysis_result.get("analysis_duration", 0),
                "security_score": analysis_result.get("todo_compliance", {}).get("score", 0),
                "vulnerabilities_found": len(analysis_result.get("security_analysis", {})),
                "agent_version": "AVAI_CanisterAgent_v4.0",
                "extraction_success": analysis_result.get("extraction_success", False),
                "analysis_method": analysis_result.get("analysis_method", "unknown")
            }
            
            # Get the report file path from the report generator
            report_filepath = getattr(self.report_generator, '_last_saved_filepath', None)
            
            if not report_filepath:
                # Generate a temporary filepath for upload metadata
                timestamp = time.strftime('%Y-%m-%d_%H-%M-%S')
                repo_name = self._extract_repo_name(repo_url)
                report_filepath = f"{repo_name}_audit_report_{timestamp}.md"
            
            # Upload to local DevNet first (safest option)
            logger.info("🚀 [DEVNET] Uploading to local IC replica...")
            local_result = await self.devnet_uploader.upload_report_to_devnet(
            ...
            if local_result.get("success"):
                logger.info("✅ [DEVNET] Local upload successful!")
                upload_summary = await self.devnet_uploader.get_upload_summary(local_result)
                logger.info(f"📊 [DEVNET] Upload Summary:\n{upload_summary}")
                
                # Store upload information in analysis result for future reference
                analysis_result["devnet_upload"] = {
                    "local": local_result,
                    "upload_timestamp": time.strftime('%Y-%m-%d %H:%M:%S')
                }
                
                # Optionally try playground network (if available)
                try:
                    logger.info("🎮 [DEVNET] Attempting playground network upload...")
                    playground_result = await self.devnet_uploader.upload_report_to_devnet(
                        report_filepath=report_filepath,
                        report_content=report_content,
                        metadata=metadata,
                        network="playground"
                    ...
                    if playground_result.get("success"):
                        logger.info("✅ [DEVNET] Playground upload successful!")
                        analysis_result["devnet_upload"]["playground"] = playground_result
                    else:
                        logger.warning(f"⚠️ [DEVNET] Playground upload failed: {playground_result.get('error')}")
                        
                except Exception as playground_error:
                    logger.warning(f"⚠️ [DEVNET] Playground upload error: {playground_error}")
                
            else:
                logger.warning(f"⚠️ [DEVNET] Local upload failed: {local_result.get('error')}")
                analysis_result["devnet_upload"] = {
                    "error": local_result.get("error"),
                    "attempted_networks": ["local"]
                }
            
        except Exception as e:
            logger.error(f"❌ [DEVNET] DevNet upload process failed: {e}")
            # Don't fail the entire analysis if DevNet upload fails
            analysis_result["devnet_upload"] = {
                "error": str(e),
                "status": "failed"
            }
